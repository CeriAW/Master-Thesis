...
#Load data
```{r}
mysample <- read_csv(file = "subsamplefinal_2.csv")
...
#Save
```{r}
#write.csv(df, file="englishreviews.csv")
#df <- read_csv(file="englishreviews.csv")
```
...
#Save x
```{r}
#write.csv(x, file="/Users/Ceri/Desktop/Thesis Data/Codes/POStags.csv")
#x <- read_csv(file="/Users/Ceri/Desktop/Thesis Data/Codes/POStags.csv")
```
...
#Save
```{r}
write.csv(df, file="save4.csv")
#df <- read_csv(file = "save4.csv")
#summary(pos)
```

#Clear space
```{r}
rm(Auto, CLI, elite2, spelling, test, test2, x.adjective, x.noun, x.numeric, x.verb, x.punctuation, x.adpositions, x.conjunctions, x.determiners, x.particles, x.X)
...
!!!! check one / two / more letter words

#SAVE
```{r}
write.csv(df, file="/Users/Ceri/Desktop/Thesis Data/Codes/PreparedDATAFinal.csv")
